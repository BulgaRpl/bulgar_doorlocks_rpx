RegisterNetEvent("CLIENT:MultiCharacter:CharacterSelected")
AddEventHandler('CLIENT:MultiCharacter:CharacterSelected', function(playerdata, newplayer)
  TriggerServerEvent('bulgar_doorlocks_rpx:Load')
end)
    ...
    if not IsEntityPlayingAnim(ped, "script_common@jail_cell@unlock@key", "action", 3) then
        ...
        while key do
            if IsEntityPlayingAnim(ped, "script_common@jail_cell@unlock@key", "action", 3) then
                ...
                key = false
                TriggerEvent("bulgar_doorlocks_rpx:updatedoor", GetPlayerServerId(PlayerId()), k)
                break
            end
        end
    end
...
RegisterNetEvent('bulgar_doorlocks_rpx:updatedoor')
AddEventHandler('bulgar_doorlocks_rpx:updatedoor', function(source, door)
    TriggerServerEvent("bulgar_doorlocks_rpx:updatedoorsv", source, door, function(cb) end)
end)

RegisterNetEvent('bulgar_doorlocks_rpx:changedoor')
AddEventHandler('bulgar_doorlocks_rpx:changedoor', function(doorID)
    local name   = Config.DoorList[doorID]
    name.locked = not name.locked
    TriggerServerEvent('bulgar_doorlocks_rpx:updateState', doorID, name.locked, function(cb) end)
end)
...
RegisterNetEvent('bulgar_doorlocks_rpx:setState')
AddEventHandler('bulgar_doorlocks_rpx:setState', function(doorID, state)
